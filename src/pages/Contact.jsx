  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        <motion.p
          className="max-w-3xl text-base sm:text-lg md:text-xl lg:text-2xl font-light text-white/90 leading-relaxed px-4"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Main Section */}
      <main className="py-10 sm:py-12 md:py-16 lg:py-24 px-4 sm:px-6 md:px-8">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {contactContent.regions.map((region, index) => (
              <motion.div
                key={region.name}
                className="bg-white p-5 sm:p-6 md:p-8 border border-gray-200 shadow-sm hover:border-cyan-400 transition-colors"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <h4 className="text-lg sm:text-xl font-bold text-gray-800 mb-3 sm:mb-4 border-b-2 border-cyan-600 pb-2 inline-block">{region.name}</h4>
